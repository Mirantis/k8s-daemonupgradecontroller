...
func copyLabels(labelsToCopy map[string]string) map[string]string {
	labels := map[string]string{}
	for key, value := range labelsToCopy {
		labels[key] = value
	}
	return labels
}
...
func newPodTemplateSpec(image string) api.PodTemplateSpec {
	return api.PodTemplateSpec{
		ObjectMeta: api.ObjectMeta{
			Labels:      copyLabels(simpleDaemonSetLabel),
...
func newDaemonSet(name, image string) *extensions.DaemonSet {
	return &extensions.DaemonSet{
		...
		Spec: extensions.DaemonSetSpec{
			Selector: &unversioned.LabelSelector{MatchLabels: copyLabels(simpleDaemonSetLabel)},
			Template: newPodTemplateSpec(image),
		},
	}
}
...
func TestGetOrCreatePodTemplate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "no matching element in the list",
			objs: []runtime.Object{
		...
		},
		{
			...
			objs: []runtime.Object{
				newPodTemplates([]*api.PodTemplate{
					newPodTemplate("nginx", "3"),
					newPodTemplate("yy", "2")}),
			},
			expected: "3",
		},
		{
			name: "no pod templates",
			objs: []runtime.Object{
				newPodTemplates([]*api.PodTemplate{}),
			},
			expected: "1",
	...
	for _, test := range tests {
		...
		if podTmplRevision != test.expected {
			t.Errorf("In test case `%s`, expected: %s got:%s", test.name, test.expected, podTmplRevision)
		}
	}
}
